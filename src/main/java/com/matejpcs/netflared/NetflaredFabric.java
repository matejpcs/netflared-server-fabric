...
public final class NetflaredFabric implements ModInitializer {
 ...
 private void run(CommandSourceStack source,String[] args){if(core==null){source.sendFailure(Component.literal("Netflared is not initialized."));return;}core.handle(new NetflaredCore.Sender(){public boolean admin(){return source.hasPermission(4);}public void send(String message){source.sendSuccess(()->Component.literal(message.replaceAll("§.","")),false);}},args);}
}